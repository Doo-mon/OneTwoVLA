...
# the json file can be downloaded from https://huggingface.co/datasets/Richard-Nai/onetwovla-dataset/tree/main/wild_move_to_no_vl
# ensure the dataset's path is $LEROBOT_HOME/umi/wild_move_to_no_vl
reasoning_json_path=/path/to/your/cot.json

# normalization stats
# this can only run on a single GPU.
# this code only needs to run once.
CUDA_VISIBLE_DEVICES=0 uv run scripts/compute_norm_stats.py pi0_visual_grounding --exp-name=computing-norm \
