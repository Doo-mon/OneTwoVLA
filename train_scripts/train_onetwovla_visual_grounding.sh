...
XLA_PYTHON_CLIENT_MEM_FRACTION=0.95 uv run scripts/train.py onetwovla_visual_grounding  --exp-name=${now_date}/${now_seconds}/one-two-vla-visual-grounding --batch-size=$batch_size --val-batch-size=$val_batch_size \
--reasoning_json_path $reasoning_json_path
